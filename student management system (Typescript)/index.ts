...
const addChildren = (name: string, age: number, rollno: number): void => {
    const newChildren: Children = {name, age, rollno};
...
const searchByRollNo = (rollno: number): void => {
    const found = childrens.find(
        (s) => s.rollno === rollno
    );
    if (found) {
